...
class ApiEventsRepository(){
    fun parseJasonFile(): List<CulturalEvent>{
        var culturalEventList = emptyList<CulturalEvent>().toMutableList()
        ...
        //WOW al salir de la corrutina, esta vacia la  lista????????????
        return culturalEventList.toList()
    }
}